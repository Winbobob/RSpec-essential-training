describe 'Expectation Matchers' do
	...
	describe 'truthiness matchers' do
		...
		it 'will match nil' do
			...
			expect(nil).to be(nil) #either way works. I personally prefer second one, since it follows previous syntax.
			expect(false).not_to be(nil) #just like nil?
	...
	describe 'numeric comparison matchers' do
		...
		it 'will match numeric ranges' do
			...
			expect(10).to be_within(1).of(9) #check whether 10 is in [8,10] or not
	...
	describe 'other useful matchers' do
		...
		it 'will match object types' do
			...
			expect('test').to be_kind_of(Object)
		...
		it 'will match class instances with #have_attributes' do
			...
			expect(car).to have_attributes(:make => 'Acura', :year => 2005, :color => 'red')
		end
	...
	describe 'predicate matchers' do
		it 'will match be_* to custom methods ending in ?' do
			...
			expect(product.visible?).to be(true) #exactly the same as above
		end
		...
		it 'will match have_* to custom methods like has_*?' do
			...
			#and return true/false. Can have arguments, but not requires.
			...
			#with custom methods
			class Customer
				def has_christmas_eve?; true; end
			end

			customer = Customer.new

			expect(customer).to have_christmas_eve #customer.has_christmas_eve?
			expect(customer.has_christmas_eve?).to be(true) #same as above
		end
	end

	describe 'observation matchers' do
		#Note that all of these use "expect{}", not "expect()".
		#It is a special block format that allows a precess to take place inside of the expectation.
		...
		it 'will match when events change object attributes' do
			...
			expect {hits.increment}.to change(hits, :count).by(1)
		end
		...
		xit 'will match when errors are raised' do
		...
		it 'will match when output is generated' do
			#observes output sent to $stuout or $stderr
	...
	describe 'composing matchers' do
		#some matchers accpet metchers as arguments . (new in rspec3)
		...
		it 'will match using noun-phrase aliases for matchers' do
			...
			#improve version of the previous example
