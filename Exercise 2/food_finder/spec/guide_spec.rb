...
=begin
      ...
      puts lines.length
      lines.each_with_index{|l, i|puts i.to_s + l}
      expect(lines[2]).to match(/^<{3}.*>{3}$/)
      expect(lines[4]).to match(/interactive/)
      expect(lines[6]).to match(/Actions/)
      expect(lines[8]).to match(/^<{3}.*>{3}$/)
=end
...
=end
        ...
        expect(lines[4]).to match(/interactive/)
        expect(lines[6]).to match(/Actions/)
        expect(lines[7]).to eq('Action not recognized.')
        expect(lines[8]).to match(/Actions/)
        expect(lines[10]).to match(/^<{3}.*>{3}$/)
      end
    ...
    context 'with quit action' do
      ...
      it 'outputs concluding message and exits' do
        ...
        output = capture_output{subject.launch!}
        lines = output.split("\n")
        expect(lines[-1]).to match(/Goodbye/)
      end
...
=end
        ...
        expect(lines[8]).to match(/\s{21}FIND A RESTAURANT/)
        expect(lines[10]).to match(/key phrase/)
        expect(lines[11]).to match(/tamale.+Mexican.+mex/)
      end
      ...
      it 'finds restaurants with matching name keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq("-" * 60)
        expect(lines[12]).to match(/^\sCafe\sMasala\s+Indian\s+\$30.00$/)
        expect(lines[13]).to eq("-" * 60)
      end
      ...
      it 'finds restaurants with matching cuisine keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq("-" * 60)
        expect(lines[12]).to match(/^\sHot\sTamale\s+Mexican\s+\$25.00$/)
        expect(lines[13]).to eq("-" * 60)
      end
      ...
      it 'finds restaurants with prices less than keyword' do
        ...
        lines = output.split("\n")
        expect(lines[11]).to eq("-" * 60)
        expect(lines[12]).to match(/^\sPita\sPocket\s+Fast\sFood\s+\$10.00$/)
        expect(lines[13]).to match(/^\sQuick\sCup\s+Coffee\s+\$5.00$/)
        expect(lines[14]).to match(/^\sTaste\sOf\sLittle\sItaly\s+Pizza\s+\$10.00$/)
